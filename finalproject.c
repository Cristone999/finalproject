...
void displayStudents() {
    ...
    for (int i = 0; i < studentCount; i++) {
        printf("%d\t%s\t\t%s\t\t%d\n", students[i].id, students[i].firstname,students[i].lastname,  students[i].age);
    }
}
...
void loadDataFromFile(const char *filename) {
    ...
    while (fscanf(file, "%d %s %s %d", &students[studentCount].id, students[studentCount].firstname, students[studentCount].lastname, &students[studentCount].age) != EOF) {
        studentCount++;
    }
